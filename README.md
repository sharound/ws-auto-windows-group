...
Designed for Windows admins, this PowerShell script `ws-auto-windows-group` automates the addition or removal of group members in a Windows local or AD domain group based on content in a CSV file or an LDAP query, turning a static group into a dynamic one.
...
* Settings can be supplied via command-line switches or by editing the script
  * For example, the group name can be specified via `customGroupName` in the upper [variable section within the script](#script-settings) or by running the script with `-groupName` command-line switch as in `ws-auto-windows-group.ps1 -groupName managers`
...
1. Store the script with its complete [directory structure](#Directory-Structure) in a location specified in the script (Default: C:\ws-auto-windows-group)
2. Create '01_Incoming\entrant.csv' according to the section [CSV File Schema](#CSV-File-Schema).
3. In Windows Task Scheduler, create a task that runs `ws-auto-windows-group.ps1`

### Command Example

```PowerShell
ws-auto-windows-group.ps1 -csvPath "C:\Folder\File.csv" -groupName "managers" -force $true
```
...
* The group name can be either specified in the upper [variable section of the script](#script-settings) with the variable `customGroupName` or via command-line switch `-groupName` as in `ws-auto-windows-group.ps1 -groupName managers`
...
* Parallel instances can be achieved by creating multiple copies of the script in different locations
  * For example, the below sets of the script (scriptDir) can be created, each scheduled to run as required
    * C:\ws-auto-windows-group\1, C:\ws-auto-windows-group\2, C:\ws-auto-windows-group\3...
...
Param(
    ...
    # - Example:  ws-auto-windows-group.ps1 -csvPath "C:\Folder\File.csv"
...
More settings can be modified at Editable Settings section in 'Scripts\ws-auto-windows-group.ps1':
...
#   - Example 1: "c:\ws-auto-windows-group\1" (where this script can be located at c:\ws-auto-windows-group\Scripts\1\ws-auto-windows-group.ps1)
#   - Example 2: "c:\ws-auto-windows-group\2" (and so on)
$scriptDir = "c:\ws-auto-windows-group"
...
$inputMode = "Static"
    ...
    # NOTE: The below has NO EFFECT when source file path is specified via command line: e.g. ws-auto-windows-group.ps1 -csvPath "C:\Folder\file.csv"
...
# NOTE: The below has NO EFFECT when group name parameter is specified via command line: e.g. ws-auto-windows-group.ps1 -groupName "managers"
...
# NOTE: The below has NO EFFECT when force parameter is specified via command line: e.g. ws-auto-windows-group.ps1 -force $true
...
$mailSubjectFailure = "WS-Auto-Windows-Group Member Deletion Skipped - " + $currentDateTime
$mailBodyFailure = "This is an automated message after WS-Auto-Windows-Group script skips user deletion due to the number of users being deleted reaches defined threshold:`n`n - userDeletionThreshold of $userDeletionThreshold`n`nFor details, please check the folder where the WS-Auto-Windows-Group script is executed, which is named using the same timestamp in this email subject at:`n`n - $scriptDir\03_Done\$currentDateTime`n"
